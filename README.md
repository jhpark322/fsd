...
| `visual_v2v_perception_node` | 상단 ROI 기반 상대 LED 4비트 beacon/상태/점수 인식 + 상대 거리 추정 |
...
| `led_interface_node` | LED 4개 상태별 on/off 패턴 + 점수 기반 점등 개수 표현 (Jetson GPIO/RPi.GPIO) |
...
┌─────────────────────────────────────────────────────────┐
│                                                         │
│  NORMAL_CENTER_DRIVE ──(1111 beacon 검출)► KEEP_RIGHT_APPROACH│
│         ▲                                    │          │
│         │                          ┌─────────┤          │
│         │               (상대 사라짐)│  (정지 거리 도달) │
│         │                          │         ▼          │
...
LED는 색상이 아니라 4개 LED의 on/off 패턴으로 인식한다. 패턴은 왼쪽부터 `LED1..LED4` 순서이며 `1`은 켜짐, `0`은 꺼짐이다.

처음에는 모든 LED를 켠 `1111` beacon으로 상대 차량 존재를 알린다. 카메라가 beacon을 4개 LED strip으로 인식하고 `approach_stop_dist_m` 이내에 들어오면 차량을 멈춘 뒤 상태값/점수 패턴 표시로 전환한다.

| 패턴 | 의미 |
|------|------|
| `1111` | 차량 beacon / 최고 점수 |
| `0101` | 우측통행 접근 |
| `1010` | 교착 감지 |
| `0010` | 가위바위보 협상 요청 |
| `1001` | 양보 후진 |
...
점수 기반 판단 중에는 점수 레벨을 `0100`, `1000`, `1100`, `1111`로 표시한다. 각각 0.25, 0.50, 0.75, 1.00에 해당한다.

꺼진 LED는 카메라 blob으로 보이지 않으므로, 수신 노드는 4개가 모두 켜진 beacon 프레임에서 패널 폭을 보정한 뒤 일부 LED만 켜진 패턴을 안정적으로 해석한다.
...
| LiDAR | 2D LiDAR (360°) |
| LED | 일반 LED 4개 직렬 (차량 beacon + 상태/협상/점수 4비트 패턴) |
| 하위 제어 | OpenCR |
